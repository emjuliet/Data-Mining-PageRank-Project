    ...
    def make_personalization_vector(self, query=None):
        ...
        if query is None:
            ...
            for string in range(n):
                if url_satisfies_query (self._index_to_url(string), query) == True:
                    v[string]= 1
    ...
    def power_method(self, v=None, x0=None, alpha=0.85, max_iterations=1000, epsilon=1e-6):
        ...
        with torch.no_grad():
            ...
            for i in range(max_iterations):
                ...
                # a construction
                a_mat=torch.ones(n,1)
                nondangling = torch.sparse.sum(self.P,1).indices()
                a_mat[nondangling] = 0
                
                # Power method
                dense_mat = (xprev.t()@a_mat + (1-alpha)) * v.t()
                torch.sparse.addmm(dense_mat.t(),self.P.t(), xprev)
